...
Якщо є — поверни JSON: {"has_anglicism":true,"rewritten":"..."} де rewritten — той самий текст повідомлення, але кожен виправлений фрагмент показаний як: {{український відповідник}}(було: англіцизм). Інші символи та розділові знаки збережи як у оригіналі. Відповідай лише одним JSON-об'єктом без пояснень і без markdown.`
